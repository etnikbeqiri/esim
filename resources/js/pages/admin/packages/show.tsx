            ...
            <div className="flex flex-col gap-6">
                    ...
                    <div className="flex items-start gap-4">
                        ...
                        <div>
                            ...
                            <p className="mt-0.5 text-sm text-muted-foreground">
                                {pkg.provider?.name || 'No provider'} · {pkg.country?.name || 'No country'}
                            </p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        ...
                        <Badge variant={pkg.in_stock ? 'outline' : 'destructive'}>
                ...
                <div className="grid grid-cols-4 gap-px overflow-hidden rounded-lg border bg-border">
                    <div className="bg-card p-4">
                        ...
                        <p className="mt-1 text-lg font-semibold">{formatData(pkg.data_mb)}</p>
                    </div>
                    <div className="bg-card p-4">
                        <p className="text-xs text-muted-foreground">Validity</p>
                        <p className="mt-1 text-lg font-semibold">{pkg.validity_days} days</p>
                    </div>
                    <div className="bg-card p-4">
                        ...
                        <p className="mt-1 text-lg font-semibold">
                            ...
                            {customRetailPrice !== null && (
                                <span className="ml-1.5 text-xs font-normal text-muted-foreground">(custom)</span>
                            )}
                        </p>
                    </div>
                    <div className="bg-card p-4">
                        ...
                        <p className="mt-1 text-lg font-semibold">
                            ...
                            <span className="ml-1.5 text-xs font-normal text-muted-foreground">({marginPercent}%)</span>
                        </p>
                    </div>
                </div>
                ...
                <div className="grid gap-8 lg:grid-cols-2">
                    <div>
                        <h2 className="mb-4 text-sm font-medium">Pricing Details</h2>
                        <div className="rounded-lg border">
                            <div className="divide-y">
                                <div className="flex items-center justify-between px-4 py-3">
                                    <span className="text-sm text-muted-foreground">Package ID</span>
                                    <code className="text-sm">{pkg.provider_package_id}</code>
                                </div>
                                {sourceCostPrice !== null && (
                                    <div className="flex items-center justify-between px-4 py-3">
                                        <span className="text-sm text-muted-foreground">
                                            Source Cost ({pkg.source_currency?.code || 'USD'})
                                        </span>
                                        <span className="text-sm">
                                            {pkg.source_currency?.symbol || '$'}{sourceCostPrice.toFixed(2)}
                                        </span>
                                    </div>
                                )}
                                <div className="flex items-center justify-between px-4 py-3">
                                    <span className="text-sm text-muted-foreground">Cost Price ({currencyCode})</span>
                                    <span className="text-sm">{currencySymbol}{costPrice.toFixed(2)}</span>
                                </div>
                                <div className="flex items-center justify-between px-4 py-3">
                                    <span className="text-sm text-muted-foreground">System Price ({currencyCode})</span>
                                    <span className={`text-sm ${customRetailPrice !== null ? 'text-muted-foreground line-through' : ''}`}>
                                        {currencySymbol}{retailPrice.toFixed(2)}
                                    </span>
                                </div>
                                {customRetailPrice !== null && (
                                    <div className="flex items-center justify-between px-4 py-3">
                                        <span className="text-sm text-muted-foreground">Custom Price ({currencyCode})</span>
                                        <span className="text-sm font-medium">{currencySymbol}{customRetailPrice.toFixed(2)}</span>
                                    </div>
                                )}
                                <div className="flex items-center justify-between bg-muted/30 px-4 py-3">
                                    <span className="text-sm font-medium">Effective Price</span>
                                    <span className="text-sm font-semibold">{currencySymbol}{effectivePrice.toFixed(2)}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="mb-4 flex items-center justify-between">
                            ...
                            {pkg.orders.length > 0 && (
                                <Button variant="ghost" size="sm" className="h-auto p-0 text-xs" asChild>
                                    <Link href={`/admin/orders?package_id=${pkg.id}`}>
                                        View all
