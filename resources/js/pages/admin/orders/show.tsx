    ...
    return (
        <div className="group flex items-center justify-between px-4 py-3">
            ...
            <div className="flex items-center gap-2">
                <code className="max-w-[280px] truncate rounded bg-muted px-2 py-0.5 font-mono text-xs">{value}</code>
                <Button variant="ghost" size="icon" className="h-6 w-6 shrink-0 opacity-0 group-hover:opacity-100" onClick={handleCopy}>
...
export default function OrderShow({ order, defaultCurrency }: Props) {
    ...
    function handleRetry() {
        retryForm.post(`/admin/orders/${order.uuid}/retry`, { preserveScroll: true });
    }

    function handleFail() {
        if (!confirm(trans('admin.orders.actions.mark_failed_confirm'))) return;
        failForm.post(`/admin/orders/${order.uuid}/fail`, { preserveScroll: true });
    }
                    ...
                    <div className="flex items-start gap-4">
                        ...
                        <div>
                            <div className="flex items-center gap-2">
                                ...
                                <Badge variant="secondary" className={`${getStatusBadgeClass(order.status_color)} flex items-center gap-1`}>
                                    {getStatusIcon(order.status)}
                                    {order.status_label}
                                </Badge>
                            </div>
                            <p className="mt-0.5 font-mono text-xs text-muted-foreground">{order.uuid}</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        {canRetry && (
                            <Button size="sm" onClick={handleRetry} disabled={retryForm.processing || isProcessing}>
                                {retryForm.processing ? <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> : <Play className="mr-1.5 h-3.5 w-3.5" />}
                                Retry
                            </Button>
                        )}
                        {canFail && (
                            <Button size="sm" variant="destructive" onClick={handleFail} disabled={failForm.processing}>
                                {failForm.processing ? <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> : <Ban className="mr-1.5 h-3.5 w-3.5" />}
                                Mark Failed
                            </Button>
                        )}
                    </div>
                ...
                <div className="mx-auto w-full max-w-5xl space-y-6 px-4">
                    {(isAwaitingPayment || isPendingRetry || isFailed) && (
                        <div className={`rounded-lg border p-4 ${
                            isAwaitingPayment ? 'border-yellow-200 bg-yellow-50 dark:border-yellow-900 dark:bg-yellow-950/30' :
                            isPendingRetry ? 'border-orange-200 bg-orange-50 dark:border-orange-900 dark:bg-orange-950/30' :
                            'border-red-200 bg-red-50 dark:border-red-900 dark:bg-red-950/30'
                        }`}>
                            <div className="flex items-start gap-3">
                                {isAwaitingPayment && <Clock className="h-5 w-5 shrink-0 text-yellow-600" />}
                                {isPendingRetry && <RefreshCw className="h-5 w-5 shrink-0 text-orange-600" />}
                                {isFailed && <XCircle className="h-5 w-5 shrink-0 text-red-600" />}
                    ...
                    <div className="grid grid-cols-4 gap-px overflow-hidden rounded-lg border bg-border">
                        <div className="bg-card p-4">
                            ...
                            <p className="mt-1 text-lg font-semibold">{currencySymbol}{amount.toFixed(2)}</p>
                        </div>
                        <div className="bg-card p-4">
                            <p className="text-xs text-muted-foreground">Cost</p>
                            <p className="mt-1 text-lg font-semibold text-muted-foreground">{currencySymbol}{costPrice.toFixed(2)}</p>
                        </div>
                        <div className="bg-card p-4">
                            <p className="text-xs text-muted-foreground">Profit</p>
                            <p className={`mt-1 text-lg font-semibold ${profit >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                {currencySymbol}{profit.toFixed(2)}
                            </p>
                        </div>
                        <div className="bg-card p-4">
                            <p className="text-xs text-muted-foreground">Retries</p>
                            <p className="mt-1 text-lg font-semibold">{order.retry_count}/{order.max_retries}</p>
                        </div>
                    </div>
                        ...
                        <div className="rounded-lg border bg-muted/30 p-4">
                            <div className="flex items-center gap-6 text-sm">
                                <div>
                                    ...
                                    <p className="font-medium">{currencySymbol}{Number(order.net_amount).toFixed(2)}</p>
                                </div>
                                <span className="text-muted-foreground">+</span>
                                <div>
                                    <p className="text-xs text-muted-foreground">VAT ({Number(order.vat_rate)}%)</p>
                                    <p className="font-medium">{currencySymbol}{Number(order.vat_amount).toFixed(2)}</p>
                                </div>
                                <span className="text-muted-foreground">=</span>
                                <div>
                                    <p className="text-xs text-muted-foreground">Total</p>
                                    <p className="font-medium">{currencySymbol}{amount.toFixed(2)}</p>
                                </div>
                            </div>
                        </div>
                    )}

                    {couponDiscount > 0 && order.coupon_usages && order.coupon_usages.length > 0 && (
                        <div className="rounded-lg border border-green-200 bg-green-50 p-4 dark:border-green-900 dark:bg-green-950/30">
                            <div className="flex items-center justify-between">
                                ...
                                <p className="text-lg font-semibold text-green-600 dark:text-green-400">-{currencySymbol}{couponDiscount.toFixed(2)}</p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="grid gap-6 lg:grid-cols-2">
                                ...
                                <div className="divide-y">
                                    ...
                                    {order.customer && (
                                        <>
                                            ...
                                            <div className="flex items-center justify-between px-4 py-3">
                                                ...
                                                <Link href={`/admin/customers/${order.customer.id}`} className="text-sm text-primary hover:underline">
                                                    View Profile
                                                </Link>
                                            </div>
                                        </>
                                    ...
                                    {order.ip_address && (
                                        <div className="flex items-center justify-between px-4 py-3">
                                            ...
                                            <code className="font-mono text-xs">{order.ip_address}</code>
                                        </div>
                                    )}
                                </div>
                            </div>
                        ...
                        <div>
                            <h2 className="mb-4 text-sm font-medium">Package</h2>
                            <div className="rounded-lg border">
                                {order.package ? (
                                    <div className="divide-y">
                                        <div className="flex items-center justify-between px-4 py-3">
                                            <span className="text-sm text-muted-foreground">Name</span>
                                            <Link href={`/admin/packages/${order.package.id}`} className="text-sm font-medium text-primary hover:underline">
                                                {order.package.name}
                                            </Link>
                                        </div>
                                        <div className="flex items-center justify-between px-4 py-3">
                                        ...
                                        <div className="flex items-center justify-between px-4 py-3">
                                            ...
                                            <span className="text-sm">{order.package.validity_days} days</span>
                                        </div>
                                        <div className="flex items-center justify-between px-4 py-3">
                                            <span className="text-sm text-muted-foreground">Provider</span>
                                            <span className="text-sm">{order.package.provider?.name || '—'}</span>
                                        </div>
                                        <div className="flex items-center justify-between px-4 py-3">
                                            <span className="text-sm text-muted-foreground">Region</span>
                                            <span className="text-sm">{order.package.country?.name || '—'}</span>
                                        </div>
                                    </div>
                                ) : (
                                    <div className="px-4 py-8 text-center text-sm text-muted-foreground">No package information</div>
                                )}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid gap-6 lg:grid-cols-2">
                        <div>
                            <h2 className="mb-4 text-sm font-medium">Payment</h2>
                            <div className="rounded-lg border">
                                {order.payment ? (
                                    <div className="divide-y">
                                        <div className="flex items-center justify-between px-4 py-3">
                                            <span className="text-sm text-muted-foreground">Status</span>
                                            <Badge variant={order.payment.status === 'completed' ? 'default' : 'secondary'}>{order.payment.status_label}</Badge>
                                        </div>
                                        <div className="flex items-center justify-between px-4 py-3">
                                            <span className="text-sm text-muted-foreground">Provider</span>
                                            <span className="text-sm">{order.payment.provider_label}</span>
                                        </div>
                                        <div className="flex items-center justify-between px-4 py-3">
                                            <span className="text-sm text-muted-foreground">Amount</span>
                                            <span className="text-sm font-medium">{currencySymbol}{Number(order.payment.amount).toFixed(2)}</span>
                                        </div>
                                        {order.payment.paid_at && (
                                            <div className="flex items-center justify-between px-4 py-3">
                                                <span className="text-sm text-muted-foreground">Paid At</span>
                                                <span className="text-sm">{order.payment.paid_at}</span>
                                            </div>
                                        )}
                                        {order.payment.gateway_payment_id && (
                                            <CopyableField label="Payment ID" value={order.payment.gateway_payment_id} />
                                        )}
                                    </div>
                                ) : (
                                    <div className="px-4 py-8 text-center text-sm text-muted-foreground">No payment record</div>
                                )}
                            </div>
                        </div>
                                ...
                                <div className="divide-y">
                                    ...
                                    {order.paid_at && (
                                        <div className="flex items-center justify-between px-4 py-3">
                                            ...
                                            <span className="text-sm">{order.paid_at}</span>
                                        </div>
                                    )}
                                    {order.completed_at && (
                                        <div className="flex items-center justify-between px-4 py-3">
                                            <span className="text-sm text-muted-foreground">Completed</span>
                                            <span className="text-sm">{order.completed_at}</span>
                                        </div>
                                    )}
                                    {order.next_retry_at && isPendingRetry && (
                                        <div className="flex items-center justify-between px-4 py-3">
                                            <span className="text-sm text-orange-600">Next Retry</span>
                                            <span className="text-sm">{order.next_retry_at}</span>
                                        </div>
                                    )}
                                    <div className="flex items-center justify-between px-4 py-3">
                                        <span className="text-sm text-muted-foreground">Updated</span>
                                        <span className="text-sm">{order.updated_at}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    {order.invoice && (
                        <div>
                            <h2 className="mb-4 text-sm font-medium">Invoice</h2>
                            <div className="rounded-lg border">
                                <div className="flex items-center justify-between px-4 py-3">
                                    <div>
                                        <code className="font-mono text-sm font-medium">{order.invoice.invoice_number}</code>
                                        <Badge variant="outline" className="ml-2">{order.invoice.status_label}</Badge>
                                    </div>
                                    <Button variant="outline" size="sm" asChild>
                                        <Link href={`/admin/invoices/${order.invoice.uuid}`}>View Invoice</Link>
                                    </Button>
                                </div>
                            </div>
                        </div>
                    ...
                    {order.esim_profile && (
                        <div>
                            <div className="mb-4 flex items-center justify-between">
                                <h2 className="text-sm font-medium">eSIM Profile</h2>
                                <Button variant="outline" size="sm" onClick={handleSyncEsim} disabled={syncForm.processing}>
                                    {syncForm.processing ? (
                            ...
                            <div className="rounded-lg border">
                                <div className="divide-y">
                                    <div className="flex items-center justify-between px-4 py-3">
                                        <span className="text-sm text-muted-foreground">Status</span>
                                        <div className="flex items-center gap-2">
                                            <Badge variant="secondary" className={getStatusBadgeClass(order.esim_profile.status_color || 'gray')}>
                                                {order.esim_profile.status_label || order.esim_profile.status || 'Unknown'}
                                            </Badge>
                                            {order.esim_profile.is_activated && (
                                                <Badge className="bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400">Activated</Badge>
                                            )}
                                        </div>
                                    </div>
                                    <div className="px-4 py-3">
                                        <div className="mb-2 flex items-center justify-between text-sm">
                                            ...
                                            <span className="font-mono">{order.esim_profile.data_used_mb.toFixed(1)} / {order.esim_profile.data_total_mb.toFixed(1)} MB</span>
                                        </div>
                                    ...
                                    {order.esim_profile.activated_at && (
                                        <div className="flex items-center justify-between px-4 py-3">
                                            ...
                                            <span className="text-sm">{order.esim_profile.activated_at}</span>
                                        </div>
                                    )}
                                    {order.esim_profile.expires_at && (
                                        <div className="flex items-center justify-between px-4 py-3">
                                            <span className="text-sm text-muted-foreground">Expires</span>
                                            <span className="text-sm">{order.esim_profile.expires_at}</span>
                                        </div>
                                    )}
                    ...
                    <div>
                        <h2 className="mb-4 text-sm font-medium">Identifiers</h2>
                        <div className="rounded-lg border">
                    ...
                    {order.metadata && Object.keys(order.metadata).length > 0 && (
                        <div>
                            ...
                            <div className="rounded-lg border p-4">
                                <pre className="max-h-48 overflow-auto font-mono text-xs">{JSON.stringify(order.metadata, null, 2)}</pre>
                            </div>
                        </div>
                    )}
                </div>
