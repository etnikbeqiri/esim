...
export default function ArticlesIndex({ articles, filters }: Props) {
    ...
    function handleTogglePublish(article: Article) {
        router.post(`/admin/articles/${article.id}/toggle-publish`, {}, { preserveState: true });
    }
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex flex-col gap-4 p-4">
                            ...
                            {articles.data.length === 0 ? (
                                ...
                                articles.data.map((article) => (
                                    <TableRow key={article.id}>
                                        ...
                                        <TableCell>{article.formatted_date || '-'}</TableCell>
                                        <TableCell>
                ...
                {articles.last_page > 1 && (
                    <div className="flex justify-center gap-2">
                        {Array.from({ length: Math.min(articles.last_page, 10) }, (_, i) => i + 1).map((page) => (
                            <Button
                                key={page}
                                variant={page === articles.current_page ? 'default' : 'outline'}
                                size="sm"
                                onClick={() => router.get('/admin/articles', { ...filters, page })}
                            >
                                {page}
                            </Button>
                        ))}
            ...
            <AlertDialog open={!!deleteArticle} onOpenChange={(open) => !open && setDeleteArticle(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
